  ...
  return (
   <div className={styles.formBack}>
     <div className={styles.form}>
       <p className={styles.close} onClick={handleCloseWindow}>✖</p>
       <div>
         <input type="text" placeholder='Название поста' value={title} onChange={(e) => setTitle(e.target.value)}/>
         <input type="text" placeholder='Ссылка на фотографию' value={image} onChange={(e) => setImage(e.target.value)}/>
         <input type="textarea" placeholder='Описание' value={desc} onChange={(e) => setDesk(e.target.value)}/>
         <button onClick={sendData} className={styles}>Добавить</button>
         {notifications && <p className={styles.notifications}>Ваш пост отправлен на модерацию.</p>}
       </div>
     </div>
   </div>
  )
